...
const localFilePopover = document.getElementById("local-file-popover");
const localFilePopoverPath = document.getElementById("local-file-popover-path");
const btnLocalFileOpen = document.getElementById("btn-local-file-open");
const btnLocalFileCopy = document.getElementById("btn-local-file-copy");
...
function renderMarkdown(text) {
    ...
    html = html.replace(/\n/g, '<br>');
    return linkifyEscapedLocalPaths(html);
}

function normalizeLocalFileHref(rawHref) {
    ...
    if (href.startsWith('/Users/')) {
        ...
        const fileMatch = href.match(/^file:\/\/([^/]*)(\/.*)$/i);
        if (fileMatch && (!fileMatch[1] || fileMatch[1].toLowerCase() === 'localhost')) {
            path = fileMatch[2];
        } else {
            const vscodeFileMatch = href.match(/^vscode:\/\/file(\/.*)$/i);
            if (vscodeFileMatch) path = vscodeFileMatch[1];
        }
    }

    if (!path || !path.startsWith('/Users/') || /[\u0000-\u001f\u007f]/.test(path)) {
        return null;
    }
...
function isLocalConsoleHostname(hostname) {
    return ['localhost', '127.0.0.1', '::1', '[::1]'].includes(String(hostname || '').toLowerCase());
}

function buildVSCodeFileHref(path) {
    const normalizedPath = normalizeLocalFileHref(path);
    if (!normalizedPath) return null;
    return `vscode://file${encodeURI(normalizedPath).replace(/#/g, '%23').replace(/\?/g, '%3F')}`;
}

function linkifyEscapedLocalPaths(html) {
    const pathRegex = /(\/Users\/[a-zA-Z0-9_.%:\-\/]+)/g;
    return String(html || '').replace(pathRegex, path => (
        `<span class="local-file-link" data-path="${path}">${path}</span>`
    ));
}
...
function sanitizeHTML(html) {
    ...
    // 1. Scrub unsafe elements/attributes and convert file links
    const walkScrub = (root) => {
        ...
        for (const el of nodes) {
            ...
            // Convert only local file links into controls. Never preserve a
            // vscode: URL supplied by message content.
            if (tag === 'a' && el.hasAttribute('href')) {
                const href = el.getAttribute('href');
                const normalizedPath = normalizeLocalFileHref(href);
                if (normalizedPath) {
                    const span = document.createElement('span');
                    span.className = 'local-file-link';
                    span.setAttribute('data-path', normalizedPath);
                    span.innerHTML = el.innerHTML;
                    el.parentNode.replaceChild(span, el);
                } else if (/^(?:file|vscode):/i.test(href)) {
                    el.removeAttribute('href');
                }
            }
        }
    };
    walkScrub(template.content);

    // 2. Discover plaintext paths in text nodes
    const walkText = (root) => {
        ...
        while (textWalker.nextNode()) {
            ...
            if (parentElement?.tagName === 'A') continue;
            if (parentElement?.classList?.contains('local-file-link')) continue;
            if (node.nodeValue.includes('/Users/')) {
        ...
        for (const node of nodesToReplace) {
            if (!pathRegex.test(node.nodeValue)) continue;
...
// 4. Local File Popover Logic
function showLocalFilePopover(element) {
    if (!localFilePopover) return;
    
    const path = element.getAttribute("data-path");
    if (!path) return;
    
    localFilePopoverPath.innerText = path;
    
    // Position popover
    const rect = element.getBoundingClientRect();
    localFilePopover.style.top = `${window.scrollY + rect.bottom + 8}px`;
    localFilePopover.style.left = `${Math.max(16, rect.left)}px`;
    localFilePopover.hidden = false;
    
    // Gated VS Code open feature (only on localhost)
    const openHref = isLocalConsoleHostname(window.location.hostname)
        ? buildVSCodeFileHref(path)
        : null;
    if (openHref) {
        btnLocalFileOpen.style.display = "";
        btnLocalFileOpen.href = openHref;
    } else {
        btnLocalFileOpen.style.display = "none";
        btnLocalFileOpen.removeAttribute("href");
    }
    
    btnLocalFileCopy.onclick = () => {
        copyTextToClipboard(path, btnLocalFileCopy);
    };
}

document.addEventListener("click", (e) => {
    // Hide popover if clicking outside
    if (localFilePopover && !localFilePopover.hidden && !e.target.closest("#local-file-popover")) {
        localFilePopover.hidden = true;
    }
    
    const fileLink = e.target.closest(".local-file-link");
    if (fileLink) {
        e.preventDefault();
        e.stopPropagation();
