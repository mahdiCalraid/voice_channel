...
function sanitizeHTML(html) {
    ...
    const walkScrub = (root) => {
        ...
        for (const el of nodes) {
            ...
            // Catch file:/// or vscode:/// links generated by marked
            if (tag === 'a' && el.hasAttribute('href')) {
                const href = el.getAttribute('href');
                if (href.startsWith('file://') || href.startsWith('vscode://')) {
                    const rawPath = href.replace(/^(file|vscode):\/\/\/?/, '/'); 
                    // Make sure it starts with / (file:///Users -> /Users, file://Users -> /Users)
    ...
    const walkText = (root) => {
        ...
        while (textWalker.nextNode()) {
            ...
            if (node.parentNode && node.parentNode.classList.contains('local-file-link')) continue;
...
function showLocalFilePopover(element) {
    ...
    if (isLocalhost) {
        ...
        btnLocalFileOpen.href = `vscode://file${path}`;
    } else {
